...
Máy chủ lưu tài khoản và dữ liệu đám mây trong `cloud_drive_store.json`. Khi triển khai, đặt `DATA_DIR` vào ổ đĩa bền vững và chỉ chạy một bản máy chủ; nhiều bản chạy đồng thời chưa có cơ sở dữ liệu dùng chung và có thể ghi đè dữ liệu của nhau. Cần triển khai máy chủ này với tên miền HTTPS trước khi build ứng dụng native. Script build sẽ dừng sớm nếu chưa có `VITE_API_BASE_URL` hợp lệ.
